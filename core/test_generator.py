...
""")
        ...
        context_parts.append("""
【生成结构约束】
- 业务模块内优先覆盖功能、边界、异常、状态流转、数据一致性测试。
- 每个功能点必须深挖字段/业务边界与异常：空值、最小/最大值、超限、特殊字符、重复提交、并发、权限不足、状态不满足、余额/库存/概率/次数/时间窗口不足或临界、接口失败/超时/弱网、取消/返回/中断。
- 通用 UI/UX 只作为 `# 全局UI/UX验证 [P2]` 一级节点出现一次。
- 通用兼容只作为 `# 全局兼容性验证 [P2]` 一级节点出现一次。
- 不要将同一 UI/UX、兼容或通用安全/性能检查复制到多个业务模块；与业务强绑定的安全/性能点可放在对应业务模块。
